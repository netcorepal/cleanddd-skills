...
- 实现要点：
    ...
    - 子实体分两种情况：
        - 拥有独立主键（一对多场景）：继承 Entity<TId>，定义自身强类型 ID，提供 protected 无参构造；外键（如 UserId）作为普通属性存储，EF 配置中使用 HasForeignKey 关联聚合根。
        - 无独立主键（多对多或复合主键场景）：定义为普通类，不继承 Entity<TId>，提供 protected 无参构造；EF 配置中用 HasKey(t => new { t.FkId1, t.FkId2 }) 定义复合主键。
