...
- 实现要点（聚合根）：
    ...
    - 属性 private set，默认值显式设置；包含 Deleted（软删除）与 RowVersion（乐观并发）。
    - 状态变更时使用 this.AddDomainEvent() 发布领域事件。
    - 强类型 ID 实现 IGuidStronglyTypedId（优先）或 IInt64StronglyTypedId，由 EF 值生成器自动生成，构造函数中无需手动设置。
- 实现要点（子实体）：
    - **推荐方式（拥有独立业务标识）**：继承 Entity<TId>，定义自身强类型 ID（推荐 IGuidStronglyTypedId），提供 protected 无参构造；外键（如聚合根 UserId）作为普通属性，EF 配置中使用 HasForeignKey 关联聚合根；ID 由 EF 值生成器自动生成。
    - **关联表/复合主键（仅表示两个聚合根间的关联，无独立业务标识）**：可定义为普通类（无需继承 Entity<TId>），提供 protected 无参构造，EF 配置中用 `HasKey(t => new { t.FkId1, t.FkId2 })` 定义复合主键。
- 聚合间规则：
    - 聚合之间不相互依赖，禁止直接引用其他聚合根实例；跨聚合交互使用领域事件或领域服务。
    - 子实体不跨聚合共享。
...
public class User : Entity<UserId>, IAggregateRoot
{
    ...
    // 多对多：User 与 Role 的关联（子实体无独立主键，复合主键）
    ...
    public void AssignRole(RoleId roleId, string roleName)
    {
        ...
        _roles.Add(new UserRole(roleId, roleName));
    }
}
```

示例：拥有独立主键的子实体（一对多，继承 Entity\<TId\>）
```csharp
namespace ProjectName.Domain.AggregatesModel.UserAggregate;

// 子实体有独立主键时：定义强类型 ID，继承 Entity<TId>
...
示例：无独立主键的子实体（多对多/复合主键，普通类）
```csharp
namespace ProjectName.Domain.AggregatesModel.UserAggregate;

// 子实体无独立主键时：普通类，不继承 Entity<TId>，EF 配置复合主键
public class UserRole
{
    protected UserRole() { }

    public UserId UserId { get; private set; } = default!;   // 复合主键之一
    public RoleId RoleId { get; private set; } = default!;   // 复合主键之一
...
internal class UserRoleEntityTypeConfiguration : IEntityTypeConfiguration<UserRole>
{
    public void Configure(EntityTypeBuilder<UserRole> builder)
    {
        ...
        builder.HasKey(t => new { t.UserId, t.RoleId });   // 复合主键
        builder.Property(b => b.RoleName).HasMaxLength(50).IsRequired().HasComment("角色名称");
    }
}
```
