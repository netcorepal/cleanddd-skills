...
- 实现要点（子实体）：
    - 子实体不强制继承 Entity<TId>，但推荐继承；继承时需定义自身强类型 ID（推荐 IGuidStronglyTypedId），提供 protected 无参构造；外键（如聚合根 UserId）作为普通属性，EF 配置中使用 HasForeignKey 关联聚合根；ID 由 EF 值生成器自动生成。
- 聚合间规则：
